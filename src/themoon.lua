...
Is_letter = function(char)
	return (char ~= "")
		and (
			(string.byte(char) >= 65 and string.byte(char) <= 90)
			or (string.byte(char) >= 97 and string.byte(char) <= 122)
		)
end
...
Is_symbol = function(char)
	return Lexer.single_symbols[char]
		or char == "="
		or char == "~"
		or char == "<"
		or char == ">"
...
Is_hex = function(char)
	return (char ~= "")
		and (
			(string.byte(char) >= 48 and string.byte(char) <= 57)
			or (string.byte(char) >= 65 and string.byte(char) <= 70)
		)
end
...
Read_number = function()
	-- if is Hex
	if Peek(0) == "0" and (Peek(1) == "x" or Peek(1) == "X") and Is_hex(Peek(2)) then
		N = 2
		while Is_hex(Peek(N)) do
			N = N + 1
		end
		if Peek(N) == "." then
			N = N + 1
